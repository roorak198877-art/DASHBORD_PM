...
const App: React.FC = () => {
  ...
  const fetchFromSheet = async (silent = false) => {
    if (!sheetUrl) { setIsSyncing(false); setIsCloudConnected(false); return; } 
    setIsSyncing(true);
    try {
        ...
        if (Array.isArray(data)) {
          const mapped: PMItem[] = data.map(entry => {
            ...
            return {
              ...
              deviceStatus: String(getVal(7, 'activity') || '').includes('Broken') ? 'Broken' : 'Ready'
            };
    ...
    if (isLoading) {
      return (
        <div className="min-h-screen bg-[#f1f5f9] flex flex-col items-center justify-center p-4">
          ...
          <p className="text-slate-400 font-black uppercase tracking-widest text-[10px]">Verifying Data...</p>
        </div>
      );
    }
        ...
        {publicItem ? (
          <motion.div initial="hidden" animate="visible" variants={modalAnimate} className="w-full max-w-md bg-white rounded-[3rem] shadow-4xl border border-slate-200 overflow-hidden relative z-10 text-left">
            ...
            <div className="p-8 space-y-6 overflow-y-auto max-h-[72vh] no-scrollbar pb-10">
              {publicItem.imageUrl && <img src={publicItem.imageUrl} className="w-full h-56 object-cover rounded-3xl border-4 border-slate-50 shadow-xl" alt="Proof" />}
              
              <div className="grid grid-cols-2 gap-4 text-left">
                <DataField label="Asset ID (A)" value={publicItem.id} mono />
                <DataField label="PM Status (G)" value={publicItem.status || 'Pending'} />
                <DataField label="Last PM (B)" value={formatDateDisplay(publicItem.date)} />
                <DataField label="Next PM (C)" value={formatDateDisplay(publicItem.nextPmDate)} />
                <DataField label="Dept (D)" value={publicItem.department} />
                <DataField label="User (F)" value={publicItem.personnel || '-'} />
                
                <DataField label="Login User (J)" value={isUnlocked ? (publicItem.computerUser || '-') : '********'} mono={isUnlocked} icon={isUnlocked ? <Eye size={12}/> : <EyeOff size={12}/>} />
                <DataField label="Password (K)" value={isUnlocked ? (publicItem.password || '-') : '********'} mono={isUnlocked} icon={isUnlocked ? <Eye size={12}/> : <EyeOff size={12}/>} />
                <DataField label="Server Pass (L)" value={isUnlocked ? (publicItem.serverPassword || '-') : '********'} mono={isUnlocked} icon={isUnlocked ? <Eye size={12}/> : <EyeOff size={12}/>} />
                <DataField label="Antivirus (M)" value={isUnlocked ? (publicItem.antivirus || '-') : '********'} mono={isUnlocked} icon={isUnlocked ? <Eye size={12}/> : <EyeOff size={12}/>} />
              </div>
              ...
              {!isUnlocked && (
                <form onSubmit={handlePinSubmit} className="p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100 flex flex-col gap-5 shadow-inner">
                  <div className="flex items-center gap-3"><Lock size={16} className="text-emerald-600" /><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Unlock Sensitive Data</span></div>
              ...
              <div className="p-7 bg-slate-50 rounded-[2.5rem] border border-slate-100 text-left">
                <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-4">PM Records (H)</p>
              ...
              <button onClick={() => { window.location.href = EXIT_URL; }} className="w-full py-5 bg-slate-800 text-white rounded-[2rem] font-black text-[12px] uppercase shadow-xl hover:bg-black transition-all flex items-center justify-center gap-3 active:scale-95 mt-6 mb-2"><ExternalLink size={18} /> Company Web</button>
            </div>
          </motion.div>
        ) : (
          <motion.div initial="hidden" animate="visible" variants={modalAnimate} className="text-center p-12 bg-white rounded-[4rem] shadow-5xl max-w-sm relative z-10 border border-slate-100">
            ...
            <h2 className="text-2xl font-black text-slate-800 mb-4 uppercase tracking-tighter">Asset Not Found</h2>
            <div className="p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100 mb-8 text-left space-y-4">
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Scanned ID (Normalized):</p>
               <p className="text-[16px] font-mono font-black text-emerald-600 truncate text-center">{normalizeId(publicViewId)}</p>
            </div>
            <div className="flex flex-col gap-4">
                {!isCloudConnected && (
                  <p className="text-[11px] text-amber-600 font-bold uppercase mb-2">Cloud not connected / ยังไม่ได้ตั้งค่า URL</p>
                )}
                <button onClick={() => { setIsLoading(true); fetchFromSheet().then(() => setIsLoading(false)); }} className="w-full px-8 py-6 bg-emerald-600 text-white rounded-[2.5rem] font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-3 shadow-4xl hover:scale-95 transition-all"><RefreshCw size={18} className={isSyncing ? "animate-spin" : ""}/> Sync & Retry</button>
                <button onClick={() => { window.location.href = EXIT_URL; }} className="w-full py-5 text-slate-400 font-black text-[11px] uppercase tracking-widest hover:text-emerald-600 transition-colors flex items-center justify-center gap-2"><ExternalLink size={16} /> Exit</button>
            </div>
            <p className="mt-8 text-[9px] text-slate-300 font-bold uppercase tracking-widest">Tip: Make sure GAS is deployed to "Anyone"</p>
          </motion.div>
        )}
      </div>
    );
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-[#f8fafc] relative font-sans overflow-x-hidden text-left">
        ...
        {isMobileMenuOpen && (
          <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25 }} className="fixed inset-0 z-[200] bg-slate-900 p-10 flex flex-col gap-12 no-print md:hidden shadow-4xl">
            ...
            <nav className="space-y-5 flex-1 pt-8 overflow-y-auto">
              ...
              <NavBtn icon={FileText} label="Asset List" active={activeTab === 'table'} onClick={() => { setActiveTab('table'); setIsMobileMenuOpen(false); }} />
            </nav>
            <div className="space-y-5">
              <button onClick={() => { setIsDbSettingsOpen(true); setIsMobileMenuOpen(false); }} className="w-full py-7 text-slate-400 font-black text-[11px] uppercase border border-slate-800 rounded-[3rem] flex items-center justify-center gap-3"><Settings size={18} /> Setup</button>
      ...
      <aside className="hidden md:flex w-80 bg-slate-900 p-12 flex-col gap-12 sticky top-0 h-screen z-20 no-print shadow-4xl border-r border-slate-800 text-left">
        ...
        <nav className="space-y-5 flex-1 pt-10">
          ...
          <div className="h-px bg-slate-800/50 my-10 mx-6"></div>
          <NavBtn icon={FileText} label="Asset Ledger" active={activeTab === 'table'} onClick={() => setActiveTab('table')} />
          <button onClick={() => fetchFromSheet()} disabled={isSyncing} className="w-full flex items-center gap-5 px-10 py-7 text-emerald-400 bg-emerald-950/30 rounded-[3rem] font-black text-[12px] uppercase border border-emerald-900/50 mt-12 hover:bg-emerald-900/40 transition-all shadow-3xl">
            {isSyncing ? <Loader2 size={20} className="animate-spin" /> : <RefreshCw size={20} />} Cloud Sync
          </button>
        </nav>
        <div className="space-y-5">
          <button onClick={() => setIsDbSettingsOpen(true)} className="w-full py-6 text-slate-400 font-black text-[10px] uppercase border border-slate-800 rounded-[2.5rem] flex items-center justify-center gap-3 hover:bg-slate-800"><Settings size={16} /> Setup</button>
          {userRole === 'admin' ? <button onClick={() => setUserRole('general')} className="w-full py-7 bg-rose-950/30 text-rose-500 rounded-[3rem] font-black text-[11px] uppercase border border-rose-900/30">Logout Admin</button> : <button onClick={() => setIsLoginModalOpen(true)} className="w-full py-7 bg-emerald-600 text-white rounded-[3rem] font-black text-[11px] uppercase shadow-3xl">Admin Login</button>}
        </div>
      </aside>
          ...
          {activeTab === 'dashboard' ? (
            <motion.div key="dash" variants={containerVariants} initial="hidden" animate="show" className="space-y-14">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 text-left">
                <MetricCard icon={Layers} title="Total Units" value={stats.total.toString()} subtitle="Current module" color="emerald" />
                <MetricCard icon={CheckCircle} title="Efficiency" value={`${stats.completionRate}%`} subtitle="Maintenance ratio" color="teal" />
                <MetricCard icon={Server} title="Connection" value={isCloudConnected ? "Online" : "Offline"} subtitle="Cloud database" color={isCloudConnected ? "emerald" : "amber"} />
                <MetricCard icon={Activity} title="Status" value="Active" subtitle="System ready" color="emerald" />
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 text-left">
                <div className="bg-white p-14 rounded-[4rem] shadow-4xl border border-slate-100 lg:col-span-2 text-left">
                  <h3 className="text-2xl font-black mb-14 uppercase flex items-center gap-5 tracking-tighter text-slate-800 text-left"><Monitor size={28} className="text-emerald-600" /> Units by Dept</h3>
                ...
                <div className="bg-white p-14 rounded-[4rem] shadow-4xl border border-slate-100 flex flex-col items-center text-left">
                  <h3 className="text-2xl font-black mb-14 uppercase flex items-center gap-5 tracking-tighter text-slate-800 self-start text-left"><TrendingUp size={28} className="text-emerald-600" /> PM Workload</h3>
                  ...
                  <div className="flex gap-6 mt-8">
                     <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-emerald-600"></div><span className="text-[10px] font-black uppercase text-slate-400">Done</span></div>
                     <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-amber-500"></div><span className="text-[10px] font-black uppercase text-slate-400">Doing</span></div>
                     <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-slate-400"></div><span className="text-[10px] font-black uppercase text-slate-400">Pending</span></div>
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div key="table" variants={containerVariants} initial="hidden" animate="show" className="bg-white rounded-[4rem] shadow-4xl overflow-hidden border border-slate-100 overflow-x-auto transition-all text-left">
              <table className="w-full text-left border-collapse min-w-[900px]">
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredItems.length > 0 ? filteredItems.map(it => (
                    <tr key={it.id} className="hover:bg-slate-50/70 transition-all group text-left">
                      <td className="px-14 py-12 text-left">
                        <div className="text-left"><p className="font-black text-slate-800 text-[18px] tracking-tight">{it.assetName || it.computerName || it.id}</p><p className="text-[11px] text-slate-400 font-black uppercase tracking-widest mt-1.5">{it.id} | {it.modelSpec || '-'}</p></div>
                      </td>
                      <td className="px-14 py-12 text-left">
                      ...
                      <td className="px-14 py-12 text-[13px] font-black text-slate-500 text-left">{formatDateDisplay(it.date)}</td>
                  ...
                  )) : (<tr><td colSpan={5} className="px-14 py-32 text-center"><p className="text-slate-400 font-black uppercase tracking-widest">No assets found</p></td></tr>)}
                </tbody>
              </table>
            </motion.div>
          )}
            ...
            <form onSubmit={handleSave} className="p-14 space-y-16 overflow-y-auto pb-40 text-left no-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-14 text-left">
                 <div className="md:col-span-1 space-y-5 text-left">
                    <label className="text-[12px] font-black text-slate-500 uppercase tracking-[0.25em] ml-8 flex items-center gap-4 text-left"><ImageIcon size={18} className="text-emerald-600"/> Asset Photo</label>
                    <div className="aspect-square bg-slate-50 border-4 border-dashed border-slate-200 rounded-[5rem] flex flex-col items-center justify-center overflow-hidden group relative shadow-inner">
                       {editingItem.imageUrl ? (<><img src={editingItem.imageUrl} className="w-full h-full object-cover" alt="Proof" /><div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center gap-8"><button type="button" onClick={() => cameraInputRef.current?.click()} className="p-6 bg-emerald-600 text-white rounded-full"><Camera size={36} /></button><button type="button" onClick={() => fileInputRef.current?.click()} className="p-6 bg-white text-emerald-600 rounded-full"><Upload size={36} /></button></div></>) : (<div className="flex flex-col items-center gap-10 text-center px-8"><div className="flex gap-8"><button type="button" onClick={() => cameraInputRef.current?.click()} className="p-7 bg-white text-emerald-600 rounded-[3rem] shadow-4xl"><Camera size={48} /></button><button type="button" onClick={() => fileInputRef.current?.click()} className="p-7 bg-white text-emerald-600 rounded-[3rem] shadow-4xl"><Upload size={48} /></button></div><span className="text-[12px] font-black text-slate-300 uppercase tracking-[0.4em]">Capture/Upload</span></div>)}
                       <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleImageUpload} />
                       <input ref={cameraInputRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={handleImageUpload} />
                    </div>
                 </div>
                 <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-10 text-left">
                    <FormInput label="Asset ID (A)" icon={Hash} value={editingItem.id} onChange={val => setEditingItem({...editingItem, id: val})} required placeholder="Ex: TC-001" />
                    <FormInput label="Last PM (B)" type="date" value={toISODate(editingItem.date)} onChange={val => setEditingItem({...editingItem, date: val})} required />
                    <FormInput label="Next PM (C)" value={formatDateDisplay(calculateNextPM(editingItem.date, editingItem.device))} readOnly icon={Calendar} />
                    <FormInput label="Asset Name (S)" icon={Tag} value={editingItem.assetName || ''} onChange={val => setEditingItem({...editingItem, assetName: val})} />
                    <FormInput label="Model Spec (T)" icon={Cpu} value={editingItem.modelSpec || ''} onChange={val => setEditingItem({...editingItem, modelSpec: val})} />
                    <FormInput label="Serial No. (U)" value={editingItem.serialNumber || ''} onChange={val => setEditingItem({...editingItem, serialNumber: val})} />
                    <FormInput label="Location (V)" icon={MapPin} value={editingItem.location || ''} onChange={val => setEditingItem({...editingItem, location: val})} />
                    <FormSelect label="Department (D)" value={editingItem.department} options={DEPARTMENTS} onChange={val => setEditingItem({...editingItem, department: val})} />
              ...
              <div className="space-y-10 text-left">
                <div className="flex items-center gap-5"><div className="w-2.5 h-10 bg-emerald-600 rounded-full"></div><h4 className="text-xl font-black text-slate-900 uppercase">Access & Security</h4></div>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-10 text-left">
                   <FormInput label="Username (J)" icon={Lock} value={editingItem.computerUser || ''} onChange={val => setEditingItem({...editingItem, computerUser: val})} />
                   <FormInput label="Password (K)" icon={Key} type="password" value={editingItem.password || ''} onChange={val => setEditingItem({...editingItem, password: val})} />
                   ...
                   <FormInput label="Antivirus (M)" value={editingItem.antivirus || ''} onChange={val => setEditingItem({...editingItem, antivirus: val})} />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-10 text-left">
                 <FormInput label="User (F)" value={editingItem.personnel || ''} onChange={val => setEditingItem({...editingItem, personnel: val})} />
                 <FormInput label="Technician (O)" value={editingItem.technician || ''} onChange={val => setEditingItem({...editingItem, technician: val})} />
                 <FormInput label="Hostname (I)" value={editingItem.computerName || ''} onChange={val => setEditingItem({...editingItem, computerName: val})} />
              </div>
              <div className="space-y-10 text-left">
                <div className="flex items-center gap-5"><div className="w-2.5 h-10 bg-slate-400 rounded-full"></div><h4 className="text-xl font-black text-slate-900 uppercase">Status & Warranty</h4></div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-10 text-left">
                   ...
                   <FormInput label="Warranty Exp (Q)" type="date" value={toISODate(editingItem.warrantyExpiry)} onChange={val => setEditingItem({...editingItem, warrantyExpiry: val})} />
                ...
                <FormInput label="Notes (R)" value={editingItem.notes || ''} onChange={val => setEditingItem({...editingItem, notes: val})} />
              </div>
              <div className="bg-emerald-50 p-12 rounded-[4.5rem] border-2 border-emerald-100/60 flex flex-col md:flex-row items-center justify-between gap-12 shadow-inner text-left">
                <div className="text-left w-full md:w-auto"><p className="text-[12px] font-black text-emerald-600 uppercase mb-4 tracking-[0.3em] text-left">Next PM Forecast</p><p className="text-5xl font-black text-emerald-900 tracking-tighter text-left">{editingItem.status === 'Completed' ? formatDateDisplay(calculateNextPM(editingItem.date, editingItem.device)) : 'PENDING'}</p></div>
                <div className="flex gap-8 w-full md:w-auto"><button type="button" onClick={() => setIsModalOpen(false)} className="px-14 py-7 bg-white text-slate-400 rounded-[3rem] font-black text-xs uppercase hover:text-rose-500 border border-slate-100 shadow-2xl">Cancel</button><button type="submit" className="px-20 py-7 bg-emerald-600 text-white rounded-[3rem] font-black text-xs uppercase shadow-4xl flex items-center justify-center gap-5 active:scale-95"><Database size={24} /> Commit & Sync</button></div>
              </div>
            </form>
          </motion.div>
        </div>
      ...
      {/* QR & Sync Settings Modals remain as per previous turns... */}
      <AnimatePresence>{isQrModalOpen && qrItem && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-6 bg-slate-900/85 backdrop-blur-xl no-print text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-[4.5rem] shadow-5xl w-full max-w-sm p-14 text-center relative overflow-hidden">
             ...
             <h3 className="text-3xl font-black mb-12 uppercase tracking-tight text-slate-900">Asset QR</h3>
             <div className="bg-slate-50 p-12 rounded-[4rem] border border-slate-100 inline-block mb-10 shadow-inner"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=350x350&data=${encodeURIComponent(`${appBaseUrl}?view=${qrItem.id}`)}`} alt="QR" className="w-56 h-56 rounded-3xl shadow-3xl" /></div>
             <div className="space-y-4 mb-8">
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest text-center">Test Connection</p>
                <div className="flex flex-col gap-3">
                   <button onClick={() => handleCopyTestLink(qrItem.id)} className="w-full py-5 bg-emerald-50 text-emerald-600 rounded-[2rem] font-black text-[11px] uppercase border border-emerald-100 hover:bg-emerald-100 flex items-center justify-center gap-3 active:scale-95"><Copy size={18} /> Copy Test Link</button>
                   <button onClick={() => setPublicViewId(qrItem.id)} className="w-full py-5 bg-slate-900 text-white rounded-[2rem] font-black text-[11px] uppercase shadow-4xl hover:bg-black flex items-center justify-center gap-3 active:scale-95"><Monitor size={18} /> Live View</button>
                </div>
             </div>
          </motion.div>
        </div>
      ...
      <AnimatePresence>{isDbSettingsOpen && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-6 bg-slate-900/95 backdrop-blur-3xl no-print text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-[4.5rem] p-16 w-full max-w-md space-y-12 shadow-5xl relative overflow-hidden text-center">
            <h3 className="text-3xl font-black uppercase tracking-tight text-slate-900">Cloud Link</h3>
            <FormInput label="GAS Webhook URL" value={sheetUrl} onChange={setSheetUrl} placeholder="https://script.google.com/..." />
            <div className="space-y-5 pt-8"><button onClick={() => { fetchFromSheet(); setIsDbSettingsOpen(false); }} className="w-full py-8 bg-emerald-600 text-white rounded-[3rem] font-black text-xs uppercase shadow-4xl hover:bg-emerald-700 flex items-center justify-center gap-5 active:scale-95"><RefreshCw size={22} /> Connect Now</button><button onClick={() => setIsDbSettingsOpen(false)} className="w-full text-slate-300 font-black text-[12px] uppercase hover:text-rose-500 transition-colors tracking-widest">Close</button></div>
          </motion.div>
        </div>
      )}</AnimatePresence>
      ...
      <AnimatePresence>{isLoginModalOpen && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-6 bg-slate-900/98 backdrop-blur-3xl no-print text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-[5.5rem] p-16 w-full max-w-sm space-y-14 shadow-5xl relative overflow-hidden text-center">
            <Lock size={64} className="mx-auto text-emerald-600 mb-10" /><div className="space-y-4 text-center"><h3 className="text-4xl font-black uppercase tracking-tight text-slate-900">Admin Login</h3><p className="text-[12px] font-black text-slate-400 uppercase tracking-[0.35em]">Authorized Personnel</p></div>
            <form onSubmit={handleLogin} className="space-y-12">
              <div className="space-y-7"><FormInput label="Username" value={loginForm.username} onChange={val => setLoginForm({...loginForm, username: val})} /><FormInput label="Password" type="password" value={loginForm.password} onChange={val => setLoginForm({...loginForm, password: val})} /></div>
              {loginError && <p className="text-[12px] font-black text-rose-500 uppercase tracking-widest">{loginError}</p>}
              <div className="space-y-6 pt-5"><button type="submit" className="w-full py-8 bg-emerald-600 text-white rounded-[3rem] font-black text-xs uppercase shadow-5xl active:scale-95">Verify</button><button type="button" onClick={() => setIsLoginModalOpen(false)} className="w-full text-slate-300 font-black text-[12px] uppercase hover:text-slate-900 transition-colors tracking-widest">Cancel</button></div>
            </form>
          </motion.div>
        </div>
      )}</AnimatePresence>
