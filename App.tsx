...
const sanitizeUrl = (url: string | null): string => {
  ...
  // 3. Ensure the URL starts with a valid protocol
...
const App: React.FC = () => {
    ...
    return (
      <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center p-4 relative overflow-hidden text-left">
        ...
        {publicItem ? (
          <motion.div initial="hidden" animate="visible" variants={modalAnimate} className="w-full max-w-md bg-white rounded-[2.5rem] shadow-4xl border border-slate-200 overflow-hidden relative z-10 text-left">
            ...
            <div className="p-6 md:p-8 space-y-5 overflow-y-auto max-h-[75vh] no-scrollbar pb-10 text-left">
              ...
              <div className="grid grid-cols-2 gap-3 text-left">
                ...
                <DataField label="12. Install Date / วันติดตั้ง" value={formatDateDisplay(publicItem.startDate)} />
                <DataField label="13. Warranty / ประกัน" value={formatDateDisplay(publicItem.warrantyExpiry)} />
              ...
              {!isUnlocked && (
                <form onSubmit={handlePinSubmit} className="p-5 bg-slate-50 rounded-[2rem] border border-slate-100 flex flex-col gap-4 shadow-inner text-left">
                  <div className="flex items-center gap-3 text-left text-left text-left"><Lock size={14} className="text-emerald-600" /><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest text-left uppercase">Unlock Credentials / ปลดล็อคดูข้อมูล</span></div>
                  <div className="flex gap-3 text-left text-left">
                    ...
                    <button type="submit" className="px-6 py-3 bg-emerald-600 text-white rounded-xl font-black text-[10px] uppercase shadow-lg active:scale-95 transition-all">Unlock</button>
                  </div>
                </form>
              )}
          ...
          <motion.div initial="hidden" animate="visible" variants={modalAnimate} className="text-center p-10 bg-white rounded-[3rem] shadow-5xl max-w-sm relative z-10 border border-slate-100 text-left text-left">
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans flex flex-col md:flex-row relative text-left">
      ...
      {/* --- MOBILE HEADER --- */}
      <div className="md:hidden sticky top-0 z-[100] bg-slate-900 px-6 py-5 flex items-center justify-between shadow-4xl no-print border-b border-slate-800 text-left text-left">
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed inset-0 z-[200] bg-slate-900 p-8 flex flex-col gap-10 no-print md:hidden shadow-5xl border-r border-slate-800 text-left text-left">
            ...
            <nav className="space-y-4 flex-1 pt-6 overflow-y-auto no-scrollbar text-left text-left text-left">
              <NavBtn 
                icon={Monitor} 
      ...
      <aside className="hidden md:flex w-72 bg-slate-900 p-10 flex-col gap-10 sticky top-0 h-screen z-20 no-print shadow-4xl border-r border-slate-800 text-left text-left">
        <div className="cursor-pointer text-left text-left" onClick={() => setActiveTab('dashboard')}><BrandIdentity size="lg" /></div>
        <nav className="space-y-4 flex-1 pt-8 text-left text-left">
      ...
      <main className="flex-1 p-6 md:p-16 overflow-y-auto w-full relative z-10 text-left text-left text-left">
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-8 no-print text-left text-left text-left">
          <div className="text-left text-left text-left text-left"><h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter uppercase leading-tight">{pmModule} Ops Hub / ศูนย์ควบคุม</h2><p className="text-slate-400 font-black text-[10px] uppercase tracking-[0.4em] mt-3">{COMPANY_NAME} • Maintenance Excellence / การบำรุงรักษาที่เป็นเลิศ</p></div>
          <div className="flex flex-wrap gap-4 w-full lg:w-auto text-left text-left text-left text-left">
        ...
        {activeTab === 'table' && (
          <div className="mb-8 no-print text-left text-left text-left">
            <div className="relative max-w-md text-left text-left text-left">
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search ID, S/N, Personnel... / ค้นหา..." className="w-full pl-12 pr-6 py-4 bg-white border-2 border-slate-100 rounded-[1.5rem] text-sm font-bold shadow-sm outline-none focus:border-emerald-600 transition-all text-left text-left text-left" />
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'dashboard' ? (
            <motion.div key="dash" variants={containerVariants} initial="hidden" animate="show" className="space-y-10 text-left text-left text-left text-left">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 text-left text-left">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 text-left text-left text-left">
                <div className="bg-white p-10 rounded-[3rem] shadow-4xl border border-slate-100 lg:col-span-2 text-left text-left overflow-hidden">
                  <h3 className="text-xl font-black mb-10 uppercase flex items-center gap-4 tracking-tighter text-slate-800 text-left text-left text-left text-left"><Monitor size={22} className="text-emerald-600" /> Distribution By Department / แยกตามแผนก</h3>
                ...
                <div className="bg-white p-10 rounded-[3rem] shadow-4xl border border-slate-100 flex flex-col items-center text-left text-left text-left overflow-hidden">
                  <h3 className="text-xl font-black mb-10 uppercase flex items-center gap-4 tracking-tighter text-slate-800 self-start text-left text-left text-left text-left"><TrendingUp size={22} className="text-emerald-600" /> Workload Summary / สรุปภาระงาน</h3>
                  <ResponsiveContainer width="100%" height={320}>
                    <PieChart>
            ...
            <motion.div key="table" variants={containerVariants} initial="hidden" animate="show" className="bg-white rounded-[3rem] shadow-4xl overflow-hidden border border-slate-100 overflow-x-auto transition-all text-left text-left text-left text-left">
              <table className="w-full text-left border-collapse min-w-[900px] text-left text-left">
                <thead className="bg-slate-50/80 border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] text-left text-left text-left text-left"><tr className="text-left text-left text-left"><th className="px-10 py-10">Asset ID / รหัสทรัพย์สิน</th><th className="px-10 py-10">Process Status / สถานะ</th><th className="px-10 py-10">Current Holder / ผู้ครอง</th><th className="px-10 py-10">Maintenance / รอบการดูแล</th><th className="px-10 py-10 text-center">Protocol / จัดการ</th></tr></thead>
                <tbody className="divide-y divide-slate-50 text-left text-left text-left">
                  {filteredItems.length > 0 ? filteredItems.map(it => (
                    <tr key={it.id} className="hover:bg-slate-50/70 transition-all group text-left text-left text-left">
                      <td className="px-10 py-10 text-left text-left text-left">
                        <div className="text-left text-left text-left text-left"><p className="font-black text-slate-800 text-[16px] tracking-tight">{it.assetName || it.id}</p><p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">{it.id}</p></div>
                      </td>
                      <td className="px-10 py-10 text-left text-left text-left">
                      ...
                      <td className="px-10 py-10 text-[13px] font-black text-slate-700 text-left text-left text-left">{it.personnel || '-'}</td>
                      <td className="px-10 py-10 text-[11px] font-black text-slate-500 text-left text-left text-left">
                      ...
                      <td className="px-10 py-10 text-center">
                        <div className="flex gap-4 justify-center text-left text-left text-left">
      ...
      <AnimatePresence>{isModalOpen && editingItem && (
        <div className="fixed inset-0 z-[500] flex items-end md:items-center justify-center bg-slate-900/85 backdrop-blur-xl overflow-y-auto pt-14 text-left text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-t-[3.5rem] md:rounded-[3.5rem] w-full max-w-7xl overflow-hidden flex flex-col max-h-[94vh] shadow-5xl border border-slate-200 text-left text-left text-left">
            <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-50 text-left text-left text-left text-left">
              <div className="flex items-center gap-6 text-left text-left text-left text-left"><div className="p-4 bg-emerald-600 text-white rounded-3xl shadow-4xl"><Wrench size={28} /></div><div className="text-left text-left text-left text-left"><h3 className="text-2xl font-black uppercase tracking-tight text-slate-900">{editingItem.id ? 'Modify Existing Asset / แก้ไขทรัพย์สิน' : 'New Asset Registration / ลงทะเบียนใหม่'}</h3><p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mt-1">Operational Protocol / {editingItem.id || 'Draft'}</p></div></div>
            ...
            <form onSubmit={handleSave} className="p-10 md:p-14 space-y-12 overflow-y-auto pb-40 text-left text-left text-left no-scrollbar text-left text-left">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-12 text-left text-left text-left">
                 <div className="md:col-span-1 space-y-4 text-left text-left text-left">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.25em] ml-6 flex items-center gap-3 text-left text-left text-left"><ImageIcon size={16} className="text-emerald-600"/> Proof of State / รูปภาพประกอบ</label>
                    <div className="aspect-square bg-slate-50 border-4 border-dashed border-slate-200 rounded-[4rem] flex flex-col items-center justify-center overflow-hidden group relative shadow-inner text-left text-left text-left">
                 ...
                 <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-8 text-left text-left text-left">
              ...
              <div className="space-y-8 text-left text-left">
                <div className="flex items-center gap-4 text-left text-left text-left"><div className="w-2 h-8 bg-emerald-600 rounded-full"></div><h4 className="text-lg font-black text-slate-900 uppercase">Secure Access Profile (J-M) / รหัสผ่าน</h4></div>
              ...
              <div className="space-y-8 text-left text-left">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-6 flex items-center gap-3 text-left text-left text-left text-left"><CheckSquare size={18} className="text-emerald-600" /> Operational Protocol Checklist (H) / รายการการตรวจสอบ</label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-10 bg-slate-50 rounded-[3rem] border border-slate-100 shadow-inner text-left text-left text-left text-left">
                  {(editingItem.device === 'Computer' ? COMPUTER_STANDARD_ACTIVITIES : PRINTER_STANDARD_ACTIVITIES).map((act, i) => {
                    ...
                    return (
                      <label key={i} className={`flex items-center gap-6 p-6 rounded-[2rem] border cursor-pointer transition-all duration-300 ${isChecked ? 'bg-white border-emerald-500 shadow-xl' : 'bg-white border-slate-100 hover:border-slate-300'}`}>
                        ...
                        <span className={`text-[13px] font-black leading-tight text-left text-left text-left text-left ${isChecked ? 'text-slate-900' : 'text-slate-400'}`}>{act}</span>
                      </label>
                    );
                  })}
                </div>
      ...
      <AnimatePresence>{isQrModalOpen && qrItem && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-6 bg-slate-900/85 backdrop-blur-xl no-print text-left text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-[3rem] shadow-5xl w-full max-w-sm p-12 text-center relative overflow-hidden text-center text-center text-center">
             ...
             <div className="bg-slate-50 p-10 rounded-[2.5rem] border border-slate-100 inline-block mb-8 shadow-inner text-center text-center">
               <img 
                 ...
                 alt="QR" 
                 className="w-48 h-48 rounded-2xl shadow-3xl text-center text-center" 
               />
             </div>
             <div className="space-y-3 mb-6 text-center text-center text-center">
      ...
      <AnimatePresence>{isDbSettingsOpen && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-6 bg-slate-900/95 backdrop-blur-3xl no-print text-left text-left text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-[3.5rem] p-12 md:p-14 w-full max-w-md space-y-10 shadow-5xl relative overflow-hidden text-center text-center text-center">
      ...
      <AnimatePresence>{isLoginModalOpen && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-6 bg-slate-900/98 backdrop-blur-3xl no-print text-left text-left text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-[4rem] p-12 md:p-14 w-full max-w-sm space-y-12 shadow-5xl relative overflow-hidden text-center text-center text-center">
            ...
            <form onSubmit={handleLogin} className="space-y-10 text-left text-left text-left">
              <div className="space-y-6 text-left text-left"><FormInput label="Identifier / ชื่อผู้ใช้" value={loginForm.username} onChange={val => setLoginForm({...loginForm, username: val})} /><FormInput label="Auth Key / รหัสผ่าน" type="password" value={loginForm.password} onChange={val => setLoginForm({...loginForm, password: val})} /></div>
...
const MetricCard: React.FC<{ title: string; value: string; subtitle: string; icon: React.ElementType; color: 'emerald' | 'teal' | 'rose' | 'amber' }> = ({ title, value, subtitle, icon: Icon, color }) => {
  ...
  return (
    <motion.div variants={bouncyItem} className="bg-white p-8 md:p-10 rounded-[2.5rem] border border-slate-100 shadow-3xl text-left text-left text-left relative z-10 overflow-hidden text-left">
      <div className={`p-4 md:p-5 rounded-xl inline-block mb-8 ${themes[color] || themes.emerald} border shadow-inner text-left text-left text-left`}><Icon size={26} /></div>
      <p className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase mb-3 tracking-[0.25em] text-left text-left text-left">{title}</p>
      <h4 className="text-3xl md:text-4xl font-black text-slate-900 truncate tracking-tighter text-left text-left text-left leading-none">{value}</h4>
      <p className="text-[9px] text-slate-400 font-black uppercase mt-4 opacity-70 text-left text-left text-left tracking-wide">{subtitle}</p>
    </motion.div>
  );
};
...
const FormInput: React.FC<{ label: string; value: string; onChange?: (val: string) => void; type?: string; placeholder?: string; required?: boolean; icon?: any; readOnly?: boolean }> = ({ label, value, onChange, type = "text", placeholder, required, icon: Icon, readOnly = false }) => (
  <div className="space-y-4 text-left text-left text-left text-left">
    <label className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-[0.25em] ml-6 flex items-center gap-3 text-left text-left text-left text-left">{Icon && <Icon size={14} className="text-emerald-600" />}{label}</label>
    <input type={type} value={value || ''} onChange={e => !readOnly && onChange?.(e.target.value)} placeholder={placeholder} required={required} readOnly={readOnly} className={`w-full px-7 py-5 md:py-6 ${readOnly ? 'bg-slate-100 text-slate-500 cursor-not-allowed font-black' : 'bg-slate-50 border-slate-100 focus:border-emerald-600 focus:bg-white'} border-2 rounded-[1.8rem] text-sm font-bold outline-none transition-all shadow-inner text-left text-left text-left`} />
  </div>
);

const FormSelect: React.FC<{ label: string; value: string; options: string[]; onChange: (val: string) => void }> = ({ label, value, options, onChange }) => (
  <div className="space-y-4 text-left text-left text-left">
    <label className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-[0.25em] ml-6 text-left text-left text-left">{label}</label>
    <div className="relative text-left text-left text-left">
      <select value={value || ''} onChange={e => onChange(e.target.value)} className="w-full px-7 py-5 md:py-6 bg-slate-50 border-2 border-slate-100 rounded-[1.8rem] text-sm font-bold outline-none appearance-none cursor-pointer focus:border-emerald-600 focus:bg-white shadow-inner transition-all text-left text-left text-left">
...
const DataField: React.FC<{ label: string; value: string; mono?: boolean; small?: boolean; icon?: React.ReactNode }> = ({ label, value, mono = false, small = false, icon }) => (
  <div className="bg-slate-50/50 p-5 md:p-6 rounded-[1.8rem] border border-slate-100 shadow-sm text-left text-left group hover:bg-white transition-all cursor-default overflow-hidden text-left text-left">
    <p className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase mb-2 tracking-widest flex items-center gap-2 text-left text-left text-left leading-none uppercase"><span className="w-1.5 h-1.5 bg-slate-300 rounded-full group-hover:bg-emerald-400 transition-colors"></span>{label}{icon && <span className="ml-auto text-emerald-400">{icon}</span>}</p>
    <p className={`font-black text-slate-800 leading-none text-left text-left text-left ${mono ? 'font-mono text-[14px] tracking-tight uppercase' : small ? 'text-[11px]' : 'text-[13px]'}`}>{value || '-'}</p>
  </div>
);
