...
// --- HELPER FUNCTIONS ---
// ปรับปรุงการค้นหา ID ให้ฉลาดขึ้น: ไม่สนช่องว่าง, ไม่สนขีด, ไม่สนพิมพ์เล็ก-ใหญ่
const normalizeId = (id: any): string => {
  if (id === null || id === undefined) return '';
  return String(id)
    .replace(/[^a-zA-Z0-9]/g, '') // เอาทุกอย่างออกเหลือแค่ตัวอักษรและตัวเลข
    .toLowerCase()
    .trim();
};
...
const App: React.FC = () => {
  ...
  // จัดการ URL Cloud: ถ้ามีใน Link ให้ทับค่าเดิมในเครื่องทันที
      ...
      if (res.ok) { 
        ...
        if (Array.isArray(data)) {
          const mapped: PMItem[] = data.map(entry => {
            ...
            // กู้คืน Activity: ล้างช่องว่าง และเครื่องหมายที่ไม่จำเป็น
            let rawActivity = String(getVal(7, 'activity') || '').trim();
          ...
          if (!silent) setSyncMessage('Cloud Data Updated / อัปเดตข้อมูลสำเร็จ');
        }
      } else {
        setIsCloudConnected(false);
      }
  ...
  const pushToCloud = async (item: PMItem) => {
    ...
    try {
      ...
      setSyncMessage("Cloud Save Success");
      setIsCloudConnected(true);
    } catch (err) { 
      setSyncMessage("Cloud Sync Error"); 
      setIsCloudConnected(false);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!finalId) return alert('Asset ID required');
  ...
  const handleCopyTestLink = (assetId: string) => {
    ...
    navigator.clipboard.writeText(fullUrl).then(() => {
      setSyncMessage("Asset Link Copied!");
      setTimeout(() => setSyncMessage(null), 2000);
    });
  };
    ...
    if (isLoading) {
      return (
        <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center justify-center p-4">
          ...
          <p className="text-slate-400 font-black uppercase tracking-widest text-[10px]">Loading Asset Details...</p>
        </div>
      );
    }
        ...
        {publicItem ? (
          <motion.div initial="hidden" animate="visible" variants={modalAnimate} className="w-full max-w-md bg-white rounded-[3rem] shadow-4xl border border-slate-200 overflow-hidden relative z-10 text-left">
            ...
            <div className="p-8 space-y-6 overflow-y-auto max-h-[72vh] no-scrollbar pb-10">
              ...
              <div className="p-7 bg-slate-50 rounded-[2.5rem] border border-slate-100 text-left">
                <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-4">PM Records / บันทึก (H)</p>
                <div className="space-y-3">
                  ...
                  {(!publicItem.activity || publicItem.activity.trim() === '') && <p className="text-[11px] text-slate-400 italic">No activity logs found</p>}
                </div>
              </div>
              <button onClick={() => { window.location.href = EXIT_URL; }} className="w-full py-5 bg-slate-800 text-white rounded-[2rem] font-black text-[12px] uppercase shadow-xl hover:bg-black transition-all flex items-center justify-center gap-3 active:scale-95 mt-6 mb-2"><ExternalLink size={18} /> Company Website</button>
            </div>
          </motion.div>
        ) : (
          <motion.div initial="hidden" animate="visible" variants={modalAnimate} className="text-center p-12 bg-white rounded-[4rem] shadow-5xl max-w-sm relative z-10 border border-slate-100">
            ...
            <div className="p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100 mb-8 text-left space-y-4">
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Searching ID:</p>
               <p className="text-[16px] font-mono font-black text-emerald-600 truncate text-center uppercase tracking-tight">{publicViewId}</p>
            </div>
            {!isCloudConnected && (
               <p className="text-[10px] text-amber-600 font-bold uppercase mb-6 tracking-widest">Cloud Not Linked / เชื่อมต่อคลาวด์ไม่ได้</p>
            )}
            <div className="flex flex-col gap-4">
                ...
                <button onClick={() => setIsDbSettingsOpen(true)} className="w-full py-4 text-slate-400 font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 hover:text-emerald-600 transition-colors"><Settings size={14}/> Setup Connection</button>
                <button onClick={() => { window.location.href = EXIT_URL; }} className="w-full py-5 text-slate-300 font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-2"><ExternalLink size={16} /> Exit System</button>
            </div>
          </motion.div>
        )}
      </div>
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-[#f8fafc] relative font-sans overflow-x-hidden text-left">
      ...
      <aside className="hidden md:flex w-80 bg-slate-900 p-12 flex-col gap-12 sticky top-0 h-screen z-20 no-print shadow-4xl border-r border-slate-800 text-left">
        ...
        <nav className="space-y-5 flex-1 pt-10">
          ...
          <button onClick={() => fetchFromSheet()} disabled={isSyncing} className="w-full flex items-center gap-5 px-10 py-7 text-emerald-400 bg-emerald-950/30 rounded-[3rem] font-black text-[12px] uppercase border border-emerald-900/50 mt-12 hover:bg-emerald-900/40 transition-all shadow-3xl">
            {isSyncing ? <Loader2 size={20} className="animate-spin" /> : <RefreshCw size={20} />} Force Sync
          </button>
        </nav>
        <div className="space-y-5">
      ...
      <main className="flex-1 p-8 md:p-20 overflow-y-auto w-full relative z-10 text-left">
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-14 gap-10 no-print text-left">
          <div className="text-left"><h2 className="text-4xl font-black text-slate-900 tracking-tighter uppercase leading-none leading-tight">PM Hub / {pmModule === 'computer' ? 'Computer' : 'Printer'}</h2><p className="text-slate-400 font-black text-xs uppercase tracking-[0.4em] mt-4">{COMPANY_NAME} • Management Console</p></div>
          ...
          {activeTab === 'dashboard' ? (
            <motion.div key="dash" variants={containerVariants} initial="hidden" animate="show" className="space-y-14">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 text-left">
                <MetricCard icon={Layers} title="Total Assets" value={stats.total.toString()} subtitle="Current module" color="emerald" />
                <MetricCard icon={CheckCircle} title="Efficiency" value={`${stats.completionRate}%`} subtitle="Maintenance ratio" color="teal" />
                <MetricCard icon={Server} title="Sync Status" value={isCloudConnected ? "Online" : "Offline"} subtitle="Cloud database" color={isCloudConnected ? "emerald" : "amber"} />
                <MetricCard icon={Activity} title="Health" value="Active" subtitle="System ready" color="emerald" />
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 text-left">
                <div className="bg-white p-14 rounded-[4rem] shadow-4xl border border-slate-100 lg:col-span-2 text-left overflow-hidden">
                  <h3 className="text-2xl font-black mb-14 uppercase flex items-center gap-5 tracking-tighter text-slate-800 text-left"><Monitor size={28} className="text-emerald-600" /> Dept Distribution</h3>
                ...
                <div className="bg-white p-14 rounded-[4rem] shadow-4xl border border-slate-100 flex flex-col items-center text-left">
                  <h3 className="text-2xl font-black mb-14 uppercase flex items-center gap-5 tracking-tighter text-slate-800 self-start text-left"><TrendingUp size={28} className="text-emerald-600" /> PM Forecast</h3>
            ...
            <motion.div key="table" variants={containerVariants} initial="hidden" animate="show" className="bg-white rounded-[4rem] shadow-4xl overflow-hidden border border-slate-100 overflow-x-auto transition-all text-left">
              <table className="w-full text-left border-collapse min-w-[900px]">
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredItems.length > 0 ? filteredItems.map(it => (
                    <tr key={it.id} className="hover:bg-slate-50/70 transition-all group text-left">
                      <td className="px-14 py-12 text-left">
                        <div className="text-left"><p className="font-black text-slate-800 text-[18px] tracking-tight truncate max-w-[200px]">{it.assetName || it.computerName || it.id}</p><p className="text-[11px] text-slate-400 font-black uppercase tracking-widest mt-1.5">{it.id} | {it.modelSpec || '-'}</p></div>
                      </td>
                      <td className="px-14 py-12 text-left">
                  ...
                  )) : (<tr><td colSpan={5} className="px-14 py-32 text-center"><p className="text-slate-400 font-black uppercase tracking-widest">No assets matching current filter</p></td></tr>)}
                </tbody>
              </table>
            </motion.div>
          )}
            ...
            <form onSubmit={handleSave} className="p-14 space-y-16 overflow-y-auto pb-40 text-left no-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-14 text-left">
                 ...
                 <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-10 text-left">
                    <FormInput label="Asset ID (A)" icon={Hash} value={editingItem.id} onChange={val => setEditingItem({...editingItem, id: val})} required placeholder="Ex: TTG-01" />
              ...
              <div className="space-y-10 text-left">
                <div className="flex items-center gap-5"><div className="w-2.5 h-10 bg-emerald-600 rounded-full"></div><h4 className="text-xl font-black text-slate-900 uppercase tracking-tight">Security Access Keys</h4></div>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-10 text-left">
                   <FormInput label="OS Username (J)" icon={Lock} value={editingItem.computerUser || ''} onChange={val => setEditingItem({...editingItem, computerUser: val})} />
                   <FormInput label="OS Password (K)" icon={Key} type="password" value={editingItem.password || ''} onChange={val => setEditingItem({...editingItem, password: val})} />
                   <FormInput label="Admin Pass (L)" icon={Key} type="password" value={editingItem.serverPassword || ''} onChange={val => setEditingItem({...editingItem, serverPassword: val})} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-10 text-left">
                 <FormInput label="Holder Name (F)" value={editingItem.personnel || ''} onChange={val => setEditingItem({...editingItem, personnel: val})} />
                 <FormInput label="Lead Tech (O)" value={editingItem.technician || ''} onChange={val => setEditingItem({...editingItem, technician: val})} />
                 <FormInput label="Hostname (I)" value={editingItem.computerName || ''} onChange={val => setEditingItem({...editingItem, computerName: val})} />
              </div>

              {/* CHECKLIST: พยายามกู้คืน Activity ให้สมบูรณ์ที่สุด */}
              <div className="space-y-10 text-left">
                <label className="text-[12px] font-black text-slate-500 uppercase tracking-[0.3em] ml-8 flex items-center gap-4 text-left"><CheckSquare size={20} className="text-emerald-600" /> PM Standard Activities (H)</label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-12 bg-slate-50 rounded-[4rem] border border-slate-100 shadow-inner text-left">
                  {(editingItem.device === 'Computer' ? COMPUTER_STANDARD_ACTIVITIES : PRINTER_STANDARD_ACTIVITIES).map((act, i) => {
                    // ตรวจสอบสถานะการติ๊ก: ล้างค่าเว้นวรรคให้เป๊ะ
                    const cleanAct = act.trim();
                    const isChecked = String(editingItem.activity || '').split('|').map(a => a.trim()).includes(cleanAct);
                    
                    return (
                      <label key={i} className={`flex items-center gap-8 p-7 rounded-[2.5rem] border cursor-pointer text-left transition-all duration-300 ${isChecked ? 'bg-white border-emerald-500 shadow-2xl translate-x-1.5' : 'bg-white border-slate-100 hover:border-slate-300'}`}>
              ...
              <div className="bg-emerald-50 p-12 rounded-[4.5rem] border-2 border-emerald-100/60 flex flex-col md:flex-row items-center justify-between gap-12 shadow-inner text-left">
                <div className="text-left w-full md:w-auto"><p className="text-[12px] font-black text-emerald-600 uppercase mb-4 tracking-[0.3em] text-left">Next PM Forecast</p><p className="text-5xl font-black text-emerald-900 tracking-tighter text-left leading-none">{editingItem.status === 'Completed' ? formatDateDisplay(calculateNextPM(editingItem.date, editingItem.device)) : 'WAITING DONE'}</p></div>
                <div className="flex gap-8 w-full md:w-auto"><button type="button" onClick={() => setIsModalOpen(false)} className="px-14 py-7 bg-white text-slate-400 rounded-[3rem] font-black text-xs uppercase hover:text-rose-500 border border-slate-100 shadow-2xl transition-all">Discard</button><button type="submit" className="px-20 py-7 bg-emerald-600 text-white rounded-[3rem] font-black text-xs uppercase shadow-4xl flex items-center justify-center gap-5 active:scale-95 hover:bg-emerald-700 transition-all"><Database size={24} /> Save & Cloud Sync</button></div>
              </div>
            </form>
          </motion.div>
        </div>
      ...
      {/* QR MODAL: ฝัง Cloud URL ลงไปใน QR ทันที */}
        ...
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-6 bg-slate-900/85 backdrop-blur-xl no-print text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-[4.5rem] shadow-5xl w-full max-w-sm p-14 text-center relative overflow-hidden">
             ...
             <div className="space-y-4 mb-8">
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest text-center">Smart Link Active (URL Included)</p>
                <div className="flex flex-col gap-3">
                   <button onClick={() => handleCopyTestLink(qrItem.id)} className="w-full py-5 bg-emerald-50 text-emerald-600 rounded-[2rem] font-black text-[11px] uppercase border border-emerald-100 hover:bg-emerald-100 flex items-center justify-center gap-3 active:scale-95 transition-all"><Copy size={18} /> Copy Shareable Link</button>
      ...
      <AnimatePresence>{isDbSettingsOpen && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-6 bg-slate-900/95 backdrop-blur-3xl no-print text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-[4.5rem] p-16 w-full max-w-md space-y-12 shadow-5xl relative overflow-hidden text-center">
            <h3 className="text-3xl font-black uppercase tracking-tight text-slate-900">Cloud Link Setup</h3>
            <FormInput label="Google Script (GAS) URL" value={sheetUrl} onChange={setSheetUrl} placeholder="https://script.google.com/macros/s/..." />
            <div className="space-y-5 pt-8"><button onClick={() => { fetchFromSheet(); setIsDbSettingsOpen(false); }} className="w-full py-8 bg-emerald-600 text-white rounded-[3rem] font-black text-xs uppercase shadow-4xl hover:bg-emerald-700 flex items-center justify-center gap-5 active:scale-95 transition-all"><RefreshCw size={22} /> Test & Connect</button><button onClick={() => setIsDbSettingsOpen(false)} className="w-full text-slate-300 font-black text-[12px] uppercase hover:text-rose-500 transition-colors tracking-widest">Discard Changes</button></div>
          </motion.div>
        </div>
      )}</AnimatePresence>
      ...
      <AnimatePresence>{isLoginModalOpen && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-6 bg-slate-900/98 backdrop-blur-3xl no-print text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-[5.5rem] p-16 w-full max-w-sm space-y-14 shadow-5xl relative overflow-hidden text-center">
            <Lock size={64} className="mx-auto text-emerald-600 mb-10" /><div className="space-y-4 text-center"><h3 className="text-4xl font-black uppercase tracking-tight text-slate-900">Staff Access</h3><p className="text-[12px] font-black text-slate-400 uppercase tracking-[0.35em]">Maintenance Department Only</p></div>
            <form onSubmit={handleLogin} className="space-y-12">
              <div className="space-y-7"><FormInput label="Admin User" value={loginForm.username} onChange={val => setLoginForm({...loginForm, username: val})} /><FormInput label="Access Pass" type="password" value={loginForm.password} onChange={val => setLoginForm({...loginForm, password: val})} /></div>
              {loginError && <p className="text-[12px] font-black text-rose-500 uppercase tracking-widest">{loginError}</p>}
              <div className="space-y-6 pt-5"><button type="submit" className="w-full py-8 bg-emerald-600 text-white rounded-[3rem] font-black text-xs uppercase shadow-5xl active:scale-95 hover:bg-emerald-700 transition-all">Authorize</button><button type="button" onClick={() => setIsLoginModalOpen(false)} className="w-full text-slate-300 font-black text-[12px] uppercase hover:text-slate-900 transition-colors tracking-widest">Cancel</button></div>
            </form>
          </motion.div>
        </div>
      )}</AnimatePresence>
