...
const sanitizeUrl = (url: string | null): string => {
  ...
  // 1. Prevents accidental appending of 'https' at the end of the URL
  // ป้องกันกรณีมีการนำ 'https' ไปต่อท้าย String เดิมโดยผิดพลาด
  if (cleanUrl.toLowerCase().endsWith('https') && cleanUrl.length > 10) {
    cleanUrl = cleanUrl.replace(/https$/i, '');
  }

  // 2. Fixes instances where URL becomes '.googhttps' into '.goog'
  ...
  // ตรวจสอบว่า URL ขึ้นต้นด้วย Protocol ที่ถูกต้องหรือไม่
...
const App: React.FC = () => {
  ...
  // Sanitize sharedCloudUrl immediately
  ...
  // sheetUrl state is always sanitized
  ...
  if (publicViewId) {
    ...
    return (
      <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center p-4 relative overflow-hidden text-left">
        <SpinningGears />
        {publicItem ? (
          <motion.div initial="hidden" animate="visible" variants={modalAnimate} className="w-full max-w-md bg-white rounded-[2.5rem] shadow-4xl border border-slate-200 overflow-hidden relative z-10 text-left text-left">
            ...
            <div className="p-6 md:p-8 space-y-5 overflow-y-auto max-h-[75vh] no-scrollbar pb-10 text-left text-left">
              ...
              {!isUnlocked && (
                <form onSubmit={handlePinSubmit} className="p-5 bg-slate-50 rounded-[2rem] border border-slate-100 flex flex-col gap-4 shadow-inner text-left text-left">
                  <div className="flex items-center gap-3 text-left text-left"><Lock size={14} className="text-emerald-600" /><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest text-left">Enter PIN to Unlock / กรอกรหัสเพื่อดูรหัสผ่าน</span></div>
                  <div className="flex gap-3 text-left">
                    ...
                    <button type="submit" className="px-6 py-3 bg-emerald-600 text-white rounded-xl font-black text-[10px] uppercase active:scale-95 transition-all shadow-lg">Unlock / ปลดล็อค</button>
                  </div>
                </form>
              )}

              <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 text-left text-left text-left">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 text-left">Maintenance Protocol / รายการตรวจสอบ</p>
          ...
          <motion.div initial="hidden" animate="visible" variants={modalAnimate} className="text-center p-10 bg-white rounded-[3rem] shadow-5xl max-w-sm relative z-10 border border-slate-100 text-left text-left text-left">
            <AlertCircle size={64} className="text-rose-500 mx-auto mb-8" />
            <h2 className="text-xl font-black text-slate-800 mb-4 uppercase tracking-tighter text-center">Reference Not Found / ไม่พบข้อมูล</h2>
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans flex flex-col md:flex-row relative text-left text-left">
      <SpinningGears />
      ...
      {/* --- MOBILE HEADER --- */}
      <div className="md:hidden sticky top-0 z-[100] bg-slate-900 px-6 py-5 flex items-center justify-between shadow-4xl no-print border-b border-slate-800">
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed inset-0 z-[200] bg-slate-900 p-8 flex flex-col gap-10 no-print md:hidden shadow-5xl border-r border-slate-800">
            ...
            <nav className="space-y-4 flex-1 pt-6 overflow-y-auto no-scrollbar text-left">
              <NavBtn 
                icon={Monitor} 
      ...
      <aside className="hidden md:flex w-72 bg-slate-900 p-10 flex-col gap-10 sticky top-0 h-screen z-20 no-print shadow-4xl border-r border-slate-800 text-left text-left">
        ...
        <div className="space-y-4 text-left text-left">
          <button onClick={() => setIsDbSettingsOpen(true)} className="w-full py-5 text-slate-400 font-black text-[9px] uppercase border border-slate-800 rounded-[2rem] flex items-center justify-center gap-3 hover:bg-slate-800 transition-all"><Settings size={14} /> Link Config / ตั้งค่าลิงก์</button>
      ...
      <main className="flex-1 p-6 md:p-16 overflow-y-auto w-full relative z-10 text-left text-left">
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-8 no-print text-left text-left">
          <div className="text-left text-left text-left"><h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter uppercase leading-tight">{pmModule} Ops Hub / ศูนย์ควบคุม</h2><p className="text-slate-400 font-black text-[10px] uppercase tracking-[0.4em] mt-3">{COMPANY_NAME} • Maintenance Excellence / การบำรุงรักษาที่เป็นเลิศ</p></div>
          <div className="flex flex-wrap gap-4 w-full lg:w-auto text-left text-left text-left">
        ...
        {activeTab === 'table' && (
          <div className="mb-8 no-print text-left text-left">
            <div className="relative max-w-md text-left text-left">
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search ID, S/N, Personnel... / ค้นหา..." className="w-full pl-12 pr-6 py-4 bg-white border-2 border-slate-100 rounded-[1.5rem] text-sm font-bold shadow-sm outline-none focus:border-emerald-600 transition-all text-left text-left" />
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'dashboard' ? (
            <motion.div key="dash" variants={containerVariants} initial="hidden" animate="show" className="space-y-10 text-left text-left text-left">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 text-left text-left">
                <div className="bg-white p-10 rounded-[3rem] shadow-4xl border border-slate-100 lg:col-span-2 text-left overflow-hidden">
                  <h3 className="text-xl font-black mb-10 uppercase flex items-center gap-4 tracking-tighter text-slate-800 text-left text-left"><Monitor size={22} className="text-emerald-600" /> Volume By Department / จำนวนแยกตามแผนก</h3>
                ...
                <div className="bg-white p-10 rounded-[3rem] shadow-4xl border border-slate-100 flex flex-col items-center text-left text-left overflow-hidden">
                  <h3 className="text-xl font-black mb-10 uppercase flex items-center gap-4 tracking-tighter text-slate-800 self-start text-left text-left"><TrendingUp size={22} className="text-emerald-600" /> Workload Summary / สรุปภาระงาน</h3>
                  <ResponsiveContainer width="100%" height={320}>
                    <PieChart>
            ...
            <motion.div key="table" variants={containerVariants} initial="hidden" animate="show" className="bg-white rounded-[3rem] shadow-4xl overflow-hidden border border-slate-100 overflow-x-auto transition-all text-left text-left text-left">
              <table className="w-full text-left border-collapse min-w-[900px] text-left">
                <thead className="bg-slate-50/80 border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] text-left text-left text-left"><tr className="text-left text-left"><th className="px-10 py-10">Asset ID / รหัสทรัพย์สิน</th><th className="px-10 py-10">Process Status / สถานะ</th><th className="px-10 py-10">Current Holder / ผู้ครอบครอง</th><th className="px-10 py-10">Maintenance / รอบการดูแล</th><th className="px-10 py-10 text-center">Protocol / จัดการ</th></tr></thead>
                <tbody className="divide-y divide-slate-50 text-left text-left">
                  {filteredItems.length > 0 ? filteredItems.map(it => (
                    <tr key={it.id} className="hover:bg-slate-50/70 transition-all group text-left text-left">
                      <td className="px-10 py-10 text-left text-left">
                      ...
                      <td className="px-10 py-10 text-[13px] font-black text-slate-700 text-left text-left">{it.personnel || '-'}</td>
                      <td className="px-10 py-10 text-[11px] font-black text-slate-500 text-left text-left">
                      ...
                      <td className="px-10 py-10 text-center">
                        <div className="flex gap-4 justify-center text-left text-left">
                          ...
                          {userRole === 'admin' && <button onClick={() => { setEditingItem({...it}); setIsModalOpen(true); }} className="p-4 text-slate-600 bg-slate-50 rounded-2xl border border-slate-100 hover:bg-emerald-600 hover:text-white transition-all shadow-sm flex items-center gap-3 active:scale-95"><Edit2 size={18} /><span className="text-[10px] font-black uppercase tracking-tight">Modify</span></button>}
                        </div>
                      </td>
                    </tr>
                  )) : (<tr><td colSpan={5} className="px-10 py-24 text-center"><p className="text-slate-400 font-black uppercase tracking-widest text-xs">No matching records found / ไม่พบข้อมูลที่ค้นหา</p></td></tr>)}
                </tbody>
              </table>
            </motion.div>
          )}
      ...
      <AnimatePresence>{isModalOpen && editingItem && (
        <div className="fixed inset-0 z-[500] flex items-end md:items-center justify-center bg-slate-900/85 backdrop-blur-xl overflow-y-auto pt-14 text-left text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-t-[3.5rem] md:rounded-[3.5rem] w-full max-w-7xl overflow-hidden flex flex-col max-h-[94vh] shadow-5xl border border-slate-200 text-left text-left">
            <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-50 text-left text-left text-left">
              <div className="flex items-center gap-6 text-left text-left"><div className="p-4 bg-emerald-600 text-white rounded-3xl shadow-4xl"><Wrench size={28} /></div><div className="text-left text-left"><h3 className="text-2xl font-black uppercase tracking-tight text-slate-900">{editingItem.id ? 'Modify Existing Asset / แก้ไขข้อมูล' : 'New Asset Registration / ลงทะเบียนใหม่'}</h3><p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mt-1">Operational Protocol / {editingItem.id || 'Draft'}</p></div></div>
            ...
            <form onSubmit={handleSave} className="p-10 md:p-14 space-y-12 overflow-y-auto pb-40 text-left text-left no-scrollbar text-left text-left">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-12 text-left text-left">
                 <div className="md:col-span-1 space-y-4 text-left text-left">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.25em] ml-6 flex items-center gap-3 text-left text-left"><ImageIcon size={16} className="text-emerald-600"/> Proof of State / รูปภาพประกอบ</label>
                    <div className="aspect-square bg-slate-50 border-4 border-dashed border-slate-200 rounded-[4rem] flex flex-col items-center justify-center overflow-hidden group relative shadow-inner">
                 ...
                 <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-8 text-left text-left">
                    ...
                    <FormInput label="Last Cycle Date (B) / วันที่ทำรอบล่าสุด" type="date" value={toISODate(editingItem.date)} onChange={val => setEditingItem({...editingItem, date: val})} required />
                    ...
                    <FormInput label="Asset Custom Name (S) / ชื่อเรียกย่อ" icon={Tag} value={editingItem.assetName || ''} onChange={val => setEditingItem({...editingItem, assetName: val})} />
                    ...
                    <FormInput label="Serial Number (U) / หมายเลขเครื่อง" value={editingItem.serialNumber || ''} onChange={val => setEditingItem({...editingItem, serialNumber: val})} />
              ...
              <div className="space-y-8 text-left text-left">
                <div className="flex items-center gap-4 text-left text-left text-left"><div className="w-2 h-8 bg-emerald-600 rounded-full"></div><h4 className="text-lg font-black text-slate-900 uppercase">Secure Access Profile / รหัสผ่านและความปลอดภัย (J-M)</h4></div>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-8 text-left text-left">
                   ...
                   <FormInput label="Endpoint Security (M) / โปรแกรมฆ่าเชื้อ" value={editingItem.antivirus || ''} onChange={val => setEditingItem({...editingItem, antivirus: val})} />
                </div>
              </div>
              ...
              <div className="bg-emerald-50 p-10 rounded-[3.5rem] border-2 border-emerald-100/60 flex flex-col md:flex-row items-center justify-between gap-10 shadow-inner text-left text-left">
                <div className="text-left text-left w-full md:w-auto"><p className="text-[10px] font-black text-emerald-600 uppercase mb-2 tracking-[0.3em]">Next Cycle Schedule / รอบการทำถัดไป</p><p className="text-4xl font-black text-emerald-900 tracking-tighter leading-none">{formatDateDisplay(calculateNextPM(editingItem.date, editingItem.device))}</p></div>
                <div className="flex gap-6 w-full md:w-auto text-left text-left text-left"><button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 md:flex-none px-10 py-6 bg-white text-slate-400 rounded-[2rem] font-black text-[10px] uppercase border border-slate-100 shadow-xl transition-all">Discard / ยกเลิก</button><button type="submit" className="flex-1 md:flex-none px-16 py-6 bg-emerald-600 text-white rounded-[2rem] font-black text-[10px] uppercase shadow-4xl flex items-center justify-center gap-4 active:scale-95 transition-all"><Database size={20} /> Commit & Sync / บันทึกข้อมูล</button></div>
              </div>
            </form>
          </motion.div>
        </div>
      ...
      <AnimatePresence>{isQrModalOpen && qrItem && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-6 bg-slate-900/85 backdrop-blur-xl no-print text-left text-left text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-[3rem] shadow-5xl w-full max-w-sm p-12 text-center relative overflow-hidden text-center text-center">
             ...
             <div className="space-y-3 mb-6 text-center">
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest text-center italic">Encryption Key Verified / ตรวจสอบความปลอดภัยแล้ว</p>
                <div className="flex flex-col gap-3 text-center">
                   <button onClick={() => { navigator.clipboard.writeText(`${appBaseUrl}?view=${qrItem.id}&url=${encodeURIComponent(sanitizeUrl(sheetUrl))}`).then(() => setSyncMessage("Link Copied / คัดลอกลิงก์สำเร็จ")); }} className="w-full py-4 bg-emerald-50 text-emerald-600 rounded-[1.5rem] font-black text-[10px] uppercase border border-emerald-100 hover:bg-emerald-100 flex items-center justify-center gap-3 active:scale-95 transition-all"><Copy size={16} /> Copy Verification Link / คัดลอกลิงก์</button>
      ...
      <AnimatePresence>{isDbSettingsOpen && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-6 bg-slate-900/95 backdrop-blur-3xl no-print text-left text-left text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-[3.5rem] p-12 md:p-14 w-full max-w-md space-y-10 shadow-5xl relative overflow-hidden text-center text-center">
            <h3 className="text-2xl font-black uppercase tracking-tight text-slate-900">Cloud Sync Config / ตั้งค่าคลาวด์</h3>
            <FormInput label="GAS Webhook URL / ลิงก์เชื่อมฐานข้อมูล" value={sheetUrl} onChange={setSheetUrl} placeholder="https://script.google.com/..." icon={Globe} />
            <div className="space-y-4 pt-6 text-center"><button onClick={() => { fetchFromSheet(); setIsDbSettingsOpen(false); }} className="w-full py-7 bg-emerald-600 text-white rounded-[2rem] font-black text-[10px] uppercase shadow-4xl hover:bg-emerald-700 flex items-center justify-center gap-4 active:scale-95 transition-all"><RefreshCw size={20} /> Establish Handshake / เชื่อมต่อระบบ</button></div>
          </motion.div>
        </div>
      )}</AnimatePresence>
      ...
      <AnimatePresence>{isLoginModalOpen && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-6 bg-slate-900/98 backdrop-blur-3xl no-print text-left text-left text-left">
          <motion.div initial="hidden" animate="visible" exit="exit" variants={modalAnimate} className="bg-white rounded-[4rem] p-12 md:p-14 w-full max-w-sm space-y-12 shadow-5xl relative overflow-hidden text-center text-center">
            <Lock size={56} className="mx-auto text-emerald-600 mb-8" /><h3 className="text-3xl font-black uppercase tracking-tight text-slate-900">Admin Gateway / เข้าสู่ระบบแอดมิน</h3>
            <form onSubmit={handleLogin} className="space-y-10 text-left text-left text-left">
              <div className="space-y-6 text-left"><FormInput label="Identifier / ชื่อผู้ใช้" value={loginForm.username} onChange={val => setLoginForm({...loginForm, username: val})} /><FormInput label="Auth Key / รหัสผ่านแอดมิน" type="password" value={loginForm.password} onChange={val => setLoginForm({...loginForm, password: val})} /></div>
...
const MetricCard: React.FC<{ title: string; value: string; subtitle: string; icon: React.ElementType; color: 'emerald' | 'teal' | 'rose' | 'amber' }> = ({ title, value, subtitle, icon: Icon, color }) => {
  ...
  return (
    <motion.div variants={bouncyItem} className="bg-white p-8 md:p-10 rounded-[2.5rem] border border-slate-100 shadow-3xl text-left text-left relative z-10 overflow-hidden">
      <div className={`p-4 md:p-5 rounded-xl inline-block mb-8 ${themes[color] || themes.emerald} border shadow-inner text-left`}><Icon size={26} /></div>
...
const FormInput: React.FC<{ label: string; value: string; onChange?: (val: string) => void; type?: string; placeholder?: string; required?: boolean; icon?: any; readOnly?: boolean }> = ({ label, value, onChange, type = "text", placeholder, required, icon: Icon, readOnly = false }) => (
  <div className="space-y-4 text-left text-left">
    <label className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-[0.25em] ml-6 flex items-center gap-3 text-left text-left">{Icon && <Icon size={14} className="text-emerald-600" />}{label}</label>
